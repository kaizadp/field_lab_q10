...
compute_q10 = function(sidb_timeseries_clean){
...
plot_q10_functions = function(q10_calculated){
...
q10_calculated_long = 
  q10_calculated %>% 
  dplyr::select(citationKey, fun, starts_with("Q10")) %>% 
  pivot_longer(-c(citationKey, fun), names_to = "temp_range", values_to = "Q10") %>% 
  mutate(temp_range = str_remove_all(temp_range, "Q10_"))

calculated_q10_graph = 
  q10_calculated_long %>% 
  filter(!is.na(fun)) %>% 
  ggplot(aes(x = temp_range, y = Q10, color = fun))+
  geom_jitter(width = 0.2,
              size = 1)+
  scale_color_manual(values = PNWColors::pnw_palette("Bay", 4))


list(q10_calculated_functions = q10_calculated_functions,
     calculated_q10_graph = calculated_q10_graph)
}
